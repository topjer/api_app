...
def get_entry(db: Session, short_url: str):
    ...
    :return:
    """
    return db.query(models.UrlData).filter(models.UrlData.short_url == short_url).first()
...
def get_long_url(db: Session, short_url: str):
    ...
    """
    result = db.query(models.UrlData).filter(models.UrlData.short_url == short_url).first()
...
def delete_entry(db: Session, short_url: str):
    result = db.query(models.UrlData).filter(models.UrlData.short_url == short_url).delete()
    db.commit()
    if result:
        return_string = f"{short_url} has been removed."
    else:
        return_string = f"{short_url} was not found in database. Nothing has been removed."
